...
function imgUploader(box, maximg, subbutton) {
    ...
    if (Comm.w9()) {
        document.getElementById(this.subbutton).onclick = function () {
            var obj = {
                url: config.root + config.appOss + '?_token=' + Comm.db("_token") + '&_device=' + _d_,
                remain: me.box ? (me.maximg - me.imgList.length) : 1
            };
            me.pickerShow && me.pickerShow();
            me.show = true;
            me.showBlock && me.showBlock(true);

            Comm.upimg(obj, function (a) {
                me.show = false;
                me.showBlock && me.showBlock(false);
                var as = a.split(",");
                var imglist = [];
...
imgUploader.prototype.addImgItem = function (src, del) {
    ...
    if (this.resize) {
        ...
        ibox.style.minWidth = 20 + "px";
        ibox.style.minHeight = 20 + "px";
    }
    ...
    Comm.confirm('您确定要删除该图片？', function (a) {
        if (parseInt(a) > 0) {
            if (me.imgList.length - 1 >= i) {
                ...
                me.delimgs(i, 1);
                if (typeof(model)!='undefined' && me.subbutton == "upload" && model.del) {
                    model.del();
...
document.write('<script src="inc/upimg/plupload.full.min.js" type="text/javascript"></script>');
...
function get_signature() {
    if (up_expire < Date.parse(new Date()) / 1000 + 3 || true) {
        ...
        if (up_token.code == 110) {
            Comm.alert("登录信息已过期，请重新登录" + (config.SLTest ? 'upload.js' : ''), function () {
                AJAX.clear();
                Comm.go('login.html')
            })
            return
