                ...
                if (o.code == 110) {
                    {
                        ...
                        exitToken();
                        app.clearUserData();
                        Comm.alert("当前用户登录已过期" + (config.SLTest ? '' : ''), function () {
                            Comm.goLogin();
                        });
                    }
                    return;
...
var Comm = new function () {
    ...
    */
    ...
    z.resizeSection = function () {
        ...
        // Comm.message('window = '+ window.innerHeight + 'section = ' + s.style.height);
    ...
    */
    ...
    z.enableGesterPop = function (enable, cb) {
        var jsonObj = {
            enable: enable
        };
        jsonObj['code'] = setcb(cb);
        if (z.w9() && z.ios()) {
            callNative('enableGesterPop', jsonObj)
        }
    };


    z.getMessageError = function (type) {
        var str = '该内容被删除';
        // if (type == 1) {
