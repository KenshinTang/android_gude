...
//标签
template.defaults.imports.replacePoint = function (s) {
    var news = s.replace(/\.0/g, "");
    return news;
}
...
template.defaults.imports.compareNum = function (a, b) {
...
template.defaults.imports.numAdd = function (a, b) {

    return parseInt(a) + parseInt(b);
}

template.defaults.imports.sphone = function (s) {
    if (Number(s) && String(s).length === 11) {
...
//根据paytype获取支付方式
template.defaults.imports.getPayType = function (state) {

    var getState = parseInt(state);
    switch (getState) {
        case 0: {
            return '未支付';
        }
...
template.defaults.imports.scorePer = function (score) {
    return Number(score) / 5 * 100;
};

template.defaults.imports.getCount = function (count) {
    return count || 0;
};


//订单状态
template.defaults.imports.getState = function (state) {
...
//距离
template.defaults.imports.distance = function (mitor) {
    var temp = '',
        km;
    mitor = Number(mitor);
    if (mitor > 1000) {
        km = (mitor / 1000).toFixed(2) + 'km';
    } else {
        km = mitor.toFixed(0) + 'm';
    }
    return km;
}
...
//根据id获取字典对象的名称
template.defaults.imports.getTag = function (tid) {
    if (!tid) return '未找到';
    var m = GDict.getItem(tid) || {};
    return m.dictname || '';
};

//保留1位小数
template.defaults.imports.onePoint = function (point) {
    return point.toFixed(1) || '0.0';
};


template.defaults.imports.getGender = function (gender) {
    if (!gender) {
        return '';
    } else if (gender == 1) {
        return '男'
...
template.defaults.imports.getLong = function (lat, lng) {

    var s = getFlatternDistance(UserJWD.JWD().lat, UserJWD.JWD().lng, lat, lng);
    if (s > 1) {
        return s + 'km';
    }
...
template.defaults.imports.getAge = function (strBirthday) {
...
template.defaults.imports.getWeekD = function (time) {
    return app.getWeekDay(time);
};

template.defaults.imports.getAPm = function (timeType) {
    if (timeType == 1) {
        return '下午';
    }
    return '上午';
...
template.defaults.imports.getText = function (v) {
...
template.defaults.imports.getReson = function (getState) {
    //0无 1手动取消 2超时取消 3未到自动取消
...
function getFlatternDistance(lat1, lng1, lat2, lng2) {
    ...
    var s = d * (1 + fl * (h1 * sf * (1 - sg) - h2 * (1 - sf) * sg)) / 1000.0;
